import require$$1$3, { Menu, BrowserWindow, app, ipcMain, nativeTheme, shell, dialog } from "electron";
import fs$1 from "fs";
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    // 窗口图标
    icon: require$$1.join(__dirname, "../public/logo.ico")
  });
  if (process.env.VITE_DEV_SERVER_URL) {
    mainWindow.loadURL(process.env.VITE_DEV_SERVER_URL);
    mainWindow.webContents.openDevTools();
  } else {
    const indexHtml = require$$1.join(__dirname, "../dist/index.html");
...
ipcMain.handle("set-native-theme", async (_, theme) => {
  try {
    ...
    console.error("Error setting native theme:", error2);
...
ipcMain.handle("get-files", async (_, dirPath) => {
  try {
    const files = await fs$1.promises.readdir(dirPath, { withFileTypes: true });
    return files.map((file2) => ({
      name: file2.name,
      isDirectory: file2.isDirectory(),
      path: require$$1.join(dirPath, file2.name)
    }));
  } catch (error2) {
  ...
  if (mainWindow) {
    try {
      ...
      if (!result.canceled && result.filePaths.length > 0) {
        ...
        try {
          await fs$1.promises.access(selectedPath, fs$1.constants.R_OK);
          event.reply("selected-folder", selectedPath);
        } catch (error2) {
          console.error("无法访问选择的文件夹:", error2);
        }
      } else {
        console.log("用户取消了文件夹选择");
      }
    } catch (error2) {
      console.error("打开文件夹对话框时出错:", error2);
    }
  } else {
...
ipcMain.handle("create-folder", async (_, folderPath) => {
  try {
    await fs$1.promises.mkdir(folderPath, { recursive: true });
    console.log("文件夹创建成功:", folderPath);
  } catch (error2) {
    console.error("创建文件夹失败:", error2);
    throw error2;
  }
});
ipcMain.handle("delete-file", async (_, filePath) => {
  try {
    ...
    if (stats.isDirectory()) {
      await fs$1.promises.rmdir(filePath, { recursive: true });
